...
export function formatPricingGuide(pricing: PublicPricingConfig): string {
  const rows = (Object.keys(PAPER_LABELS) as PaperProfileKey[])
    .map((key) => {
      ...
      return `<tr><th scope="row">${PAPER_LABELS[key]}</th><td>${formatPeso(profile.paperCost)}</td><td>${formatPeso(profile.bwPrint.low)}</td><td>${formatPeso(profile.bwPrint.medium)}</td><td>${formatPeso(profile.bwPrint.high)}</td><td>${formatPeso(profile.bwPrint.very_high)}</td><td>${formatPeso(profile.colorPrint.low)}</td><td>${formatPeso(profile.colorPrint.medium)}</td><td>${formatPeso(profile.colorPrint.high)}</td><td>${formatPeso(profile.colorPrint.very_high)}</td></tr>`;
    })
    .join('');
  ...
  return `<div class="pricing-table-wrap"><table class="pricing-table"><caption>Paper &amp; Print Pricing per Page</caption><thead><tr><th scope="col">Paper size</th><th scope="col">Bond Paper (sheet)</th><th scope="col">B&amp;W (Low: 0&ndash;10%)</th><th scope="col">B&amp;W (Med: 10&ndash;40%)</th><th scope="col">B&amp;W (High: 40&ndash;70%)</th><th scope="col">B&amp;W (Max: 70&ndash;100%)</th><th scope="col">Color (Low: 0&ndash;10%)</th><th scope="col">Color (Med: 10&ndash;40%)</th><th scope="col">Color (High: 40&ndash;70%)</th><th scope="col">Color (Max: 70&ndash;100%)</th></tr></thead><tbody>${rows}</tbody></table></div><div class="pricing-guide-footer"><div class="pricing-tier-strip"><span class="pricing-tier-tag"><span class="pricing-tier-tag__dot pricing-tier-tag__dot--low"></span><strong>Low (0&ndash;10%)</strong>: Text, forms</span><span class="pricing-tier-tag"><span class="pricing-tier-tag__dot pricing-tier-tag__dot--med"></span><strong>Medium (10&ndash;40%)</strong>: Diagrams</span><span class="pricing-tier-tag"><span class="pricing-tier-tag__dot pricing-tier-tag__dot--high"></span><strong>High (40&ndash;70%)</strong>: Charts</span><span class="pricing-tier-tag"><span class="pricing-tier-tag__dot pricing-tier-tag__dot--max"></span><strong>Very High (70&ndash;100%)</strong>: Photos</span></div><div class="pricing-notes-bar"><span class="pricing-note-pill pricing-paper-note"><strong>Bond Paper Costing:</strong> ${formatPeso(pricing.paperProfiles.a4.paperCost)}/sheet for all sizes</span>${duplexNote}<span class="pricing-note-pill pricing-quality-note"><span class="pricing-quality-note__label">High quality</span>: +${formatPeso(pricing.highQualitySurcharge)} <span class="pricing-quality-note__unit">per page</span></span></div></div>`;
}
